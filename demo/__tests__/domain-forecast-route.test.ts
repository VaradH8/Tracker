...
const LEAD_ONLY: [string, (u: unknown) => Promise<Response>][] = [
...
describe("forecast + approval route role gates", () => {
  ...
  for (const [label, call] of LEAD_ONLY) {
    ...
    it(`${label}: 403 for a Team Lead`, async () => {
  ...
  it("approving a submission that was already reviewed is a 409", async () => {
    ...
    vi.mocked(prisma.domainTagSubmission.findUnique).mockResolvedValue({
      ...
      completedCount: 70,
      assignmentId: 1,
      assignment: { assignedCount: 100, deliveredCount: 70 },
    } as never);
  ...
  it("a Lead can't approve more tags than were claimed", async () => {
    ...
    vi.mocked(prisma.domainTagSubmission.findUnique).mockResolvedValue({
      ...
      completedCount: 70,
      assignmentId: 1,
      assignment: { assignedCount: 100, deliveredCount: 0 },
    } as never);
  ...
  it("a Lead can't approve past what's left on the assignment", async () => {
    ...
    vi.mocked(prisma.domainTagSubmission.findUnique).mockResolvedValue({
      ...
      completedCount: 50,
      assignmentId: 1,
      assignment: { assignedCount: 100, deliveredCount: 80 },
    } as never);
